/*
 * Spektrafilm for Android — host parity test for the downscale (minification)
 * anti-aliasing prefilter on the crop/resize geometry stage.
 * Copyright (C) 2026 Spektrafilm Android contributors. GPLv3.
 * Port of spektrafilm (GPLv3) by Andrea Volpato — film modeling powered by spektrafilm.
 *
 * Exercises a sub-unity upscale_factor=0.5 (MINIFICATION) on the scan_film route
 * (spatial/grain off). skimage.transform.rescale defaults anti_aliasing=True for
 * an output dimension that shrinks (order>0), running scipy.ndimage.gaussian_filter
 * (sigma = max(0,(in/out-1)/2) = 0.5 per spatial axis, mode='mirror') BEFORE the
 * order=3 cubic zoom. Without that prefilter the C++ diverges from the oracle by
 * ~2e-1 max_abs; with it the whole pipeline matches within parity tolerance.
 *
 * It:
 *   1) loads the deterministic 64x64 scan_portra_input_rgb.f64 fixture,
 *   2) runs the WHOLE scan_film pipeline through spk_simulate() with
 *      upscale_factor=0.5,
 *   3) asserts the output GEOMETRY (32x32) matches the golden, and that every
 *      pixel matches goldens/scan_portra_downscale/{film_density_cmy,final_rgb}.spkvec
 *      within the standard parity tolerance (max_abs <= 1e-4, rms <= 1e-5).
 *
 * The golden is pinned to oracle SHA c1d0e44 (provia/param-wiring provenance).
 *
 * Build (host): full source set per tests/README, e.g.
 *   g++ -std=c++17 -O2 -I <cpp_root> -I <tools/parity> \
 *     tests/test_downscale.cpp spektra.cpp kernels/*.cpp io/*.cpp \
 *     model/*.cpp profiles/*.cpp runtime/*.cpp runtime/stages/*.cpp \
 *     -o /tmp/test_downscale
 */
#include <cmath>
#include <cstdint>
#include <cstdio>
...
const char* kGoldenDir =
    "/home/user/Spectrafilmandroid/tools/parity/goldens/scan_portra_downscale";
...
int main(int argc, char** argv) {
    ...
    // --- Sub-unity upscale_factor: MINIFICATION with AA prefilter -------------
    p.crop = 0;
    p.upscale_factor = 0.5f;

    // Expected output geometry from the golden (32x32).
    ...
    std::printf("Input %dx%d -> expected minified %dx%d (%d pixels)\n",
    ...
    if (pass_geom && out.width * out.height * 3 == static_cast<int>(gold_rgb.data.size()))
        pass_rgb = check("scan_portra_downscale final_rgb", out.data, gold_rgb.data);
    else
        std::printf("[scan_portra_downscale final_rgb] SKIPPED (geometry mismatch)\n");
    spk_image_free(&out);

    // film_density_cmy tap (verifies the minified negative directly).
    spk_image tap{};
    ...
    if (st != SPK_OK) {
        ...
        pass_cmy = check("scan_portra_downscale film_density_cmy", tap.data, gold_cmy.data);
        spk_image_free(&tap);
    } else {
        std::printf("[scan_portra_downscale film_density_cmy] geometry mismatch %dx%d\n",
