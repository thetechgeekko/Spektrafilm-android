...
via LibRaw — `lib/libraw/src/main/kotlin/com/spectrafilm/libraw/RawDecoder.kt`). This doc reverse-engineers the `.mcraw` format and lays out a
...
```
MainActivity.kt       rawPicker (SAF OpenDocument, no MIME filter)
MainActivity.kt       RawDecoder.isRawFileName(name)         ← extension gate
lib/libraw/src/main/kotlin/com/spectrafilm/libraw/RawDecoder.kt
                      RAW_EXTENSIONS = {dng,cr2,cr3,nef,arw,raf,orf,rw2}
EngineHelpers.kt      decodeRawToLinear() → RawDecoder.decodeToLinear(bytes/fd)
