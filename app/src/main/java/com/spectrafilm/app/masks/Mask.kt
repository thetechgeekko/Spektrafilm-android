...
sealed interface MaskComponent {
     ...
     */
    data class Radial(
        val cx: Float, val cy: Float, val rx: Float, val ry: Float, val feather: Float = 0.5f,
    ) : MaskComponent {
        override fun alphaAt(nx: Float, ny: Float): Float {
            val ex = if (rx > 1e-6f) (nx - cx) / rx else 0f
            val ey = if (ry > 1e-6f) (ny - cy) / ry else 0f
...
data class Mask(
    ...
    data class Component(val mode: BlendMode, val shape: MaskComponent)

    /** Fold the components at a normalized coordinate → final alpha in [0,1]. */
    fun alphaAt(nx: Float, ny: Float): Float {
        var a = 0f
        for ((mode, shape) in components) {
            val c = shape.alphaAt(nx, ny).coerceIn(0f, 1f)
            a = when (mode) {
