#!/usr/bin/env bash
#
# Spektrafilm for Android — spatial stages CPU-vs-GPU probe (#215). GPLv3.
# Film modeling powered by spektrafilm.
#
# Builds tools/gpu_probe/probe_spatial_main.cpp against the UNMODIFIED engine,
# pushes it, waits for the phone to cool, and runs it. Engine sources are compiled
# as-is; nothing under engine/ is modified.
#
# Usage: bash tools/gpu_probe/build_push_run_spatial.sh [W] [H]
...
"$CXX" $FLAGS tools/gpu_probe/probe_spatial_main.cpp $SRC -lvulkan -llog -landroid \
  -o "$OUT/gpu_probe_spatial"

echo "== push =="
adbw push "$OUT/gpu_probe_spatial" /data/local/tmp/gpu_probe_spatial >/dev/null
adbw shell chmod 755 /data/local/tmp/gpu_probe_spatial
...
LOG="$CAP/spatial_${STAMP}.txt"
echo "== run =="
adbw shell /data/local/tmp/gpu_probe_spatial | tee "$LOG"
echo
echo "capture: $LOG"
