#!/usr/bin/env bash
#
# Spektrafilm for Android — f32-vs-f64 FFT convolution probe. GPLv3.
# Film modeling powered by spektrafilm.
#
# Builds tools/gpu_probe/probe_f32_fft_main.cpp against the UNMODIFIED engine,
...
"$CXX" $FLAGS tools/gpu_probe/probe_f32_fft_main.cpp $SRC -lvulkan -llog -landroid \
  -o "$OUT/gpu_probe_f32fft"

echo "== push =="
adbw push "$OUT/gpu_probe_f32fft" /data/local/tmp/gpu_probe_f32fft >/dev/null
adbw shell chmod 755 /data/local/tmp/gpu_probe_f32fft
...
LOG="$CAP/f32fft_${STAMP}.txt"
echo "== run =="
adbw shell /data/local/tmp/gpu_probe_f32fft | tee "$LOG"
echo
echo "capture: $LOG"
